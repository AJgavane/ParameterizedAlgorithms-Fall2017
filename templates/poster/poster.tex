...
\input{tex/general_graphs.tex}

% Vertical spaces of 0.2in give a good buffer
\vspace{0.2in}

% Here's a nice way of using a mid-section header
\begin{center}
\Large
\textbf{Complexity Hierarchy}
\hrule
\end{center}
Reduce computational complexity by introducing a parameter
\begin{itemize}
	\item \emph{Natural parameter}:  Size of solution (e.g.~a clique of size $k$)
	\item \emph{Structural parameter}:  Graph invariant (e.g.~treewidth $k$)
\end{itemize}

\begin{center}
	{\input{tikz/hierarchy.tex}}
\end{center}
\begin{center}
	\small
	\textbf{Figure:  Relationship between various complexity classes.  It is unknown whether there are problems in NP that do not admit any polynomial time algorithm.}
\end{center}
...
Great Ideas!
\end{center}
Diverse set of FPT techniques, including:
\begin{itemize}
	\item \textbf{Kernelization:}  Transform instance into a smaller, equivalent instance of size $f(k)$ in polynomial time
	\item \textbf{Randomization:}  Random branching will give correct solution with high probability after $f(k)$ trials
	\item \textbf{Iterative Compression:} Starting with a solution of size $k+1$, quickly find a solution of size $k$ or smaller
\end{itemize}
